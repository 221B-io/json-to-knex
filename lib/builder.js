...
function validateColumn(col) {
  ...
  if (
    (col.inTable || col.onDelete || col.references) &&
    !(col.inTable && col.onDelete && col.references)
  ) {
    throw "When a 'inTable', 'onDelete', or 'references' field is specified on a column, the other fields must be specified as well.";
  }
}
