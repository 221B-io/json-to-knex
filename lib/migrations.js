...
/**
 * Returns a list of tables to drop, reversed from the order they were added.
 * @param {JSON} oldSchema
 * @param {JSON} newSchema
 */
function getTablesToDrop(oldSchema, newSchema) {
  return _.differenceBy(oldSchema.tables, newSchema.tables, "name")
    .map(x => x.name)
    .reverse();
}
...
async function updateSchema(knex, oldSchema, newSchema) {
  ...
  // For each table in the new schema, find the same table in the old schema and updates from the old to the new. If it doesn't exist in the old schema, create it instead of altering any existing table.
  for (let i in newSchema.tables) {
    let t = newSchema.tables[i];
    // if not in oldSchema
    let matches = oldSchema.tables.filter(x => x.name === t.name);
    if (matches.length === 0) {
      await builder.createTable(knex.schema, t.name, t);
    } else if (matches.length > 1) {
      console.log(`ERROR: table name ${t.name} is not unique!`);
    } else {
      // if matches.length === 1
      // if in oldSchema
      console.log(str(matches[0]));
      await updateTable(knex, t.name, matches[0], t);
    }
  }
}
